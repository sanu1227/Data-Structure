...
void printcmp(struct LOL_Dic champ) {
	printf("이름: %s\n", champ.name);
	printf("hp: %d\n", champ.hp);
	printf("mp: %d\n", champ.mp);
	printf("speed: %d\n", champ.speed);
...
void Search(int i,char searchname[]) {
	...
	if (strcmp(loldic[i].name, searchname) == 0 && loldic[i].used ==1) {
		printcmp(loldic[i]);
		return;
	}
...
int DeleteAll(int i, char delpos[]) {
	if (i >= 100) {
		return 0;
	}
	...
	return deleted + DeleteAll(i + 1, delpos);
}
...
int main() {
	...
	while (1) {
		...
		if ((strcmp(order, "End") == 0)) {
			...
			int result = DeleteAll(0, delpos);
			
			if (result == 0) {
				printf("해당 포지션의 챔피언이 없습니다\n");
			}
			else {
				printf("%d명의 챔피언이 삭제되었습니다\n", result);
			}
		}
		else if ((strcmp(order, "FindMaxHp") == 0)) {
			int maxhpindex;
			int i;
			maxhpindex = FindMaxHp(0);
			if (maxhpindex == -1) {
				printf("저장된 챔피언이 없습니다\n");
			}
			else {
				for (i = 0; i < 100; i++) {
					if (loldic[maxhpindex].hp == loldic[i].hp) {
						printcmp(loldic[i]);
					}
				}
			}
